jQuery(window).on('load', function(){
    ...
    window[ 'sd_display_more_info' ] = function( e ){
        e = $( e );
        e.parent().hide().siblings( '.sd_more_info' ).show();
    };

    window[ 'sd_hide_more_info' ] = function( e ){
        e = $( e );
        e.parent().hide().siblings( '.sd_more_info_hndl' ).show();
    };
    ...
    window ['open_insertion_sell_downloads_product_window'] = function(){
        var tags = sell_downloads.tags_p,
            cont = $(tags.replace(/&lt;/g, '<').replace(/&gt;/g, '>').replace(/&quot;/g, '"'));
            ...
            buttons: [
                {text: 'OK', click: function() {
                    var id  = $('#product_id').val(),
                        sc  = '[sell_downloads_product id="'+id+'"]';
    ...
    window['sd_resend_email'] = function(id){
		var f = $('#purchase_form');
    ...
    jQuery('.product-data').bind('click', function(evt){
		var t = $(evt.target);
        if(t.hasClass('button_for_upload_sd')){
            var file_path_field = t.parent().find('.file_path');
            var cfg = {
                ...
                (function( field, flag ){
                    return function() {
                        var attachment = media.state().get('selection').first().toJSON(),
							size = ('cover' in sell_downloads) ? sell_downloads['cover'] : 'medium';
						if(
							flag &&
							typeof attachment[ 'sizes' ] != 'undefined' && typeof attachment[ 'sizes' ][ size ] != 'undefined'
						)
						{
		...
		else if(t.hasClass('add_new_cover_sd'))
		{
			$('.sd-covers').append(sd_cover_template);
		}
		else if(t.hasClass('button_remove_cover_sd'))
		{
			t.closest('.sd-cover').remove();
		}
    });
